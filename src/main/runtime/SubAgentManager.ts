...
export class SubAgentManager {
  ...
  definitions(owner: string): ToolDefinition[] {
    ...
    return [
      ...
      definition('wait_agent', 'Wait until at least one listed child settles or timeout. A timeout does not cancel children. Check returned status and output before reporting success.', { ids: { type: 'array', description: 'Direct child IDs.', items: id }, timeout_ms: { type: 'integer', description: '0–300000 milliseconds, default 30000.' } }, ['ids'], (args, signal) => this.wait(owner, args.ids, args.timeout_ms ?? 30000, signal)),
