...
export class ProviderError extends IexaError {
  ...
  static from(error: unknown): ProviderError {
    if (isAbortError(error)) return new ProviderError('PROVIDER_ABORTED', error instanceof Error ? error.message : 'Request aborted', false, undefined, error);
    ...
    const retryable = status === 408 || status === 425 || status === 429 || (!!status && status >= 500) ||
      /timeout|network|socket|econn|reset|temporar|overload|stream idle|no response body|premature|quota_exceeded|fetch failed|\bterminated\b/i.test(message);
