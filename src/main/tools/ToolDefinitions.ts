  ...
  const tools: AgentToolDefinition[] = [
    ...
    },
    {
      ...
      parameters: {
        ...
        backend: { type: 'string', description: 'Select a backend from the user\'s explicit request for this task: native targets an existing desktop window, native-isolated launches a new app on a separately owned Windows desktop without moving existing windows, and chromium-cdp targets the explicitly supplied browser page. Isolated workspaces permit semantic operations only, never physical input; this is UI isolation, not a file/network sandbox.' },
        focusPolicy: { type: 'string', description: 'Per-task foreground focus behavior; default stop. stop dispatches no physical input if the target is unfocused. wait waits up to focusWaitMs for the user to naturally return focus, then takes a fresh observation. Never activates or steals focus.', enumValues: ['stop', 'wait'] },
        focusWaitMs: { type: 'integer', description: 'Maximum user-focus wait when focusPolicy=wait, 1–60000 ms; default 30000. Timeout sends no input.' },
        ...
        includeOcr: { type: 'boolean', description: 'Run local OCR only when UI controls are insufficient (default false). OCR elements expose confidence:null when the recognizer provides no calibrated confidence; cross-check the current frame before acting on OCR-only targets.' },
