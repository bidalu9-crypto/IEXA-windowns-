...
# Fix estimated flag: it should be true when the displayed number is from estimation,
# not just when we've never received an API receipt.
...
new = """  status(history: AgentMessage[], state: ContextStatus['state'] = 'ok'): ContextStatus {
    ...
    // under-report the next request's actual history.
    const usedTokens = Math.max(this.lastReportedInputTokens, estimatedTokens);
    // The label is '模型实测' only when the displayed number actually came from
    // the API. It is '估算' when the local estimate is the value we show.
