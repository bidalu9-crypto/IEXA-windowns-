const fs=require('node:fs'),path=require('node:path'),os=require('node:os'),{createHash,randomUUID}=require('node:crypto');
...
async function nativeIsolatedModel({root,out,exe,document}){
 ...
 const workspace=fs.mkdtempSync(path.join(os.tmpdir(),'IEXA-desktop-model-'));
 const taskDocument=path.join(workspace,'owned-note.txt');fs.writeFileSync(taskDocument,'IEXA baseline');
 const runtime=new ToolRuntime({workspaceDir:workspace,memoryDir:path.join(workspace,'memory'),memoryEnabled:false,permissionMode:'risk',desktopCaptureFrames:false,budget:new BudgetManager({maxTurns:16,maxToolCalls:24,maxRuntimeMs:300000,maxInputTokens:160000}),permissionResolver:async request=>request.tool.name==='desktop_control'?'allow_once':'deny'});
 ...
 fileTool.execute=(args,context)=>path.resolve(String(args.path||''))===taskDocument?readFile(args,context):Promise.resolve({success:false,output:'Read-back verification is limited to the supplied document.'});
 ...
 desktop.execute=async(args,context)=>{
  for(const step of [args,...(args.action==='batch'&&Array.isArray(args.actions)?args.actions:[])]){
   ...
   if(['activate','minimize','move','drag','key','hotkey','scroll'].includes(step.action))return {success:false,output:'Physical input and activation are outside this task.'};
 ...
 try{
  ...
  report.goal=`请在不打断我当前桌面工作的前提下，启动真实 Windows 记事本并打开文件 ${taskDocument}，把全部正文替换成这一行：${reference}，然后保存到原文件并核对结果。控件与菜单请自行观察定位，只用 desktop_control 操作软件；可以用 file_read 读回这一目标文件核验保存，禁止用文件写入或 shell 代替软件操作。`;
  save();timer=setTimeout(()=>{report.deadlineExceeded=true;controller.abort();},300000);
  ...
  report.diskText=fs.readFileSync(taskDocument,'utf8').replace(/^\uFEFF/,'');
  report.businessVerified=report.diskText===reference;
  report.backendSelectionVerified=report.actions.find(a=>a.name==='desktop_control')?.args.backend==='native-isolated';
  ...
  report.status=report.businessVerified&&report.backendSelectionVerified&&report.observationEvidence.length&&report.readBackEvidence.length&&!report.error&&!report.cancelled?'passed':'failed';
  ...
  if(path.dirname(path.resolve(workspace))===path.resolve(os.tmpdir())&&path.basename(workspace).startsWith('IEXA-desktop-model-'))fs.rmSync(workspace,{recursive:true,force:true});
